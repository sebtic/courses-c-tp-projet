 ...
 *
 * $Id: Quotation.c 247 2010-09-10 10:23:07Z sebtic $
 */
...
void Quotation_save(Document * document) {
    char buf[1024];
    snprintf(buf, 1024, BASEPATH "/data/quotation-%s.dat", document->docNumber);
...
void Quotation_load(const char * docNumber, Document * document) {
    char buf[1024];
    snprintf(buf, 1024, BASEPATH "/data/quotation-%s.dat", docNumber);
    Document_loadFromFile(document, buf);
}

static GtkTreeModel * Quotation_loadModel(void) {
    GtkListStore * store;
    GtkTreeIter iter;
    GDir * directory;
    Document document;
    ...
    if (directory == NULL) {
        GtkWidget * errordialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
                GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Impossible de lister les devis");
        ...
        while ((entry = g_dir_read_name(directory)) != NULL) {
            if (icaseStartWith("quotation-", entry) && icaseEndWith(".dat", entry)) {
                char * filename = concatenateString(BASEPATH "/data/", entry);
                Document_loadFromFile(&document, filename);
                gtk_list_store_append(store, &iter);
                gtk_list_store_set(store, &iter, 0, document.docNumber, 1, document.customer.name,
                        2, document.editDate, 3, document.object, -1);
                free(filename);
            }
        }
...
static int Quotation_select(Document * document) {
    GtkWidget *dialog;
    int result = 0;
    GtkWidget * sw;
    ...
    dialog = gtk_dialog_new_with_buttons("Choix d'un devis", NULL, GTK_DIALOG_MODAL
            | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_OK, GTK_RESPONSE_OK, GTK_STOCK_CLOSE,
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
        ...
        if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
            gchar * docNumberGTK;
            gtk_tree_model_sort_convert_iter_to_child_iter(GTK_TREE_MODEL_SORT(sortedmodel),
                    &childIter, &iter);
            gtk_tree_model_get(model, &childIter, 0, &docNumberGTK, -1);
            Quotation_load(docNumberGTK, document);
            g_free(docNumberGTK);
            result = 1;
        }
    }
...
void Quotation_new(GtkWidget * UNUSED( widget), GtkWindow * parent) {
    Document document;
    time_t curTime;
    struct tm * tm;
    ...
    if (DocumentEditor_edit(parent, NEW_DOCUMENT, &document))
        Quotation_save(&document);

    Document_finalize(&document);
}

void Quotation_open(GtkWidget * UNUSED( widget), GtkWindow * parent) {
    Document document;
...
void Quotation_display(GtkWidget * UNUSED( widget), GtkWindow * parent) {
    Document document;
...
void Quotation_convertToBill(GtkWidget * UNUSED( widget), GtkWindow * parent) {
    Document document;
    time_t curTime;
    struct tm * tm;
